...
@section('page-title', 'Past Questions Store')
...
<div class="store-header">
  <div>
    <h2>📝 Past Questions Store</h2>
    <div class="store-count">{{ $items->total() }} past question {{ $items->total() == 1 ? 'set' : 'sets' }} available</div>
  </div>
  <a href="{{ route('dashboard') }}" class="ws-cta" style="position:static;z-index:auto;">← Back to Dashboard</a>
</div>

<form method="GET" action="{{ route('store.past-questions') }}" class="store-search" style="margin-bottom:20px;">
  <div class="search-wrap" style="max-width:400px;">
    ...
    <button type="submit" class="btn-store primary" style="flex:0 0 auto;padding:8px 16px;border-radius:8px;background:#2563EB;">Search</button>
  </div>
</form>
...
<div class="store-grid">
  ...
  <div class="store-card">
    ...
    <div class="store-meta">Previous exam papers & solutions</div>
    <div class="store-price">Free</div>
    <div class="store-actions">
      <a href="{{ route('store.past-questions.download', $item->id) }}" class="btn-store primary">Download</a>
    </div>
  </div>
  @endforeach
</div>
...
<div class="empty-store">
  ...
  <p>No past questions available right now. Check back soon!</p>
</div>
@endif
@endsection
