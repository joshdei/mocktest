...
@section('page-title', 'Materials Store')
...
<div class="store-header">
  <div>
    <h2>📚 Materials Store</h2>
    <div class="store-count">{{ $items->total() }} course material {{ $items->total() == 1 ? 'set' : 'sets' }} available</div>
  </div>
  <a href="{{ route('dashboard') }}" class="ws-cta" style="position:static;z-index:auto">← Back to Dashboard</a>
</div>

<form method="GET" action="{{ route('store.materials') }}" class="store-search" style="margin-bottom:20px;">
  <div class="search-wrap" style="max-width:400px;">
    ...
    <button type="submit" class="btn-store primary" style="flex:0 0 auto;padding:8px 16px;border-radius:8px;background:#D97706;">Search</button>
  </div>
</form>
...
<div class="store-grid">
  ...
  <div class="store-card">
    ...
    {{-- <div class="store-name">{{ $item->course_code }} Materials</div> --}}
    

   @php
    $fullPath = '/home/psalmedu/public_html/public/storage/' . $item->file_path;
  
    $fileExists = file_exists($fullPath);
@endphp
    <div class="store-meta">Lecture notes, handouts & resources</div>
    <div class="store-price">Free</div>
    @if(!$fileExists)
    <div class="missing-msg">File not available (not uploaded yet)</div>
    @endif
    <div class="store-actions">
      @if($fileExists)
      <a href="{{ route('store.materials.download', $item->id) }}" class="btn-store primary">Download</a>
      @else
      <span class="btn-store disabled">Not Available</span>
      @endif
    </div>
  </div>
  @endforeach
...
<div class="empty-store">
  ...
  <p>No materials available right now. Check back soon!</p>
</div>
@endif
@endsection
